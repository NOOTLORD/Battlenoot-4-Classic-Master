class HudWDoubleDomination extends HudCDoubleDomination;
//Positioning
...
simulated function ShowTeamScorePassA(Canvas C)
{
	...
	if ( bShowPoints )
	{
		DrawWidgetAsTile (C, SymbolGB[0]);
		DrawWidgetAsTile (C, SymbolGB[1]);

		DrawWidgetAsTile (C, DomPoints[0]);
		DrawWidgetAsTile (C, DomPoints[1]);
		...
		DrawWidgetAsTile (C, Points[0].Widgets[Points[0].PointState]);
		DrawWidgetAsTile (C, Points[1].Widgets[Points[1].PointState]);

		if ( DP1 != None )
		{
			C.DrawColor = HudColorHighLight;
			if (bCorrectAspectRatio)
				Draw2DLocationDot(C, DP1.Location,0.5 - REDtmpPosX*HUDScale * (ResScaleY/ResScaleX), REDtmpPosY*HUDScale, REDtmpScaleX*HUDScale*(ResScaleY/ResScaleX), REDtmpScaleY*HUDScale);
			else Draw2DLocationDot(C, DP1.Location,0.5 - REDtmpPosX*HUDScale, REDtmpPosY*HUDScale, REDtmpScaleX*HUDScale , REDtmpScaleY*HUDScale);
		}
		if ( DP2 != None )
		{
			C.DrawColor = HudColorHighLight;
			if (bCorrectAspectRatio)
				Draw2DLocationDot(C, DP2.Location,0.5 + BLUEtmpPosX*HUDScale * (ResScaleY/ResScaleX), BLUEtmpPosY*HUDScale, BLUEtmpScaleX*HUDScale*(ResScaleY/ResScaleX), BLUEtmpScaleY*HUDScale);
			else	Draw2DLocationDot(C, DP2.Location,0.5 + BLUEtmpPosX*HUDScale, BLUEtmpPosY*HUDScale, BLUEtmpScaleX*HUDScale, BLUEtmpScaleY*HUDScale);
		}
