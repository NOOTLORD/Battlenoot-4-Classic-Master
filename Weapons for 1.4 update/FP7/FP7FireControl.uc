//=============================================================================
// Fire control class for the FP7 Grenade
//
...
simulated function Initialize()
{
	...
	foreach VisibleCollidingActors( class 'Actor', A, DamageRadius, Location )
	{
		if (xPawn(A)!=None)
			IgniteActor(A);
		class'BallisticDamageType'.static.Hurt(A, (1.0-(VSize(A.Location - Location)/DamageRadius)) * BaseDamage, Instigator, A.Location, Normal(A.Location - Location)*500, class'DT_FP7Immolation');
	}
	...
	for(i=0;i<MAX_FIRE_SPOTS;i++)
	{
		...
		if (GF!=None)
		{
			...
			if (bHeld)
				GF.DamageType = class'DT_FP7held';
		}
	}
}
...
simulated function IgniteActor(Actor A)
{
	...
	if (bHeld)
		PF.DamageType = class'DT_FP7held';
	PF.Initialize(A);
}
