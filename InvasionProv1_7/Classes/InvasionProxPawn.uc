...
class InvasionProxPawn extends BallisticPawn;
...
simulated exec function MenuHud()
{
	if(Controller != None)
	{
		InvasionProXPlayer(Controller).ClientOpenMenu("InvasionProv1_7.InvasionProHudConfig");
	}
}

simulated exec function MenuPet()
{
	if(InvasionProXPlayer(Controller) != None && InvasionProXPlayer(Controller).bPetMode)
	{
		InvasionProXPlayer(Controller).ClientOpenMenu("InvasionProv1_7.InvasionProPetMenuConfig");
	}
}
...
function DoComboName( string ComboClassName )
{
	...
	if(Controller != None)
	{
		...
		if ( ComboClass != None)
		{
			if(InvasionProBot(Controller).bDisableSpeed && ComboClass == class'ComboSpeed')
			{
				return;
			}
			else if(InvasionProBot(Controller).bDisableBerserk && ComboClass == class'ComboBerserk' )
			{
				return;
			}

			else if( InvasionProBot(Controller).bDisableInvis && ComboClass == class'ComboInvis' )
			{
				return;
			}
			else if( InvasionProBot(Controller).bDisableDef && ComboClass == class'ComboDefensive' )
			{
				return;
			}
...
function Died(Controller Killer, class<DamageType> damageType, vector HitLocation)
{
	...
	if(Controller != None)
	{
		StatsItem = InvasionPro(Level.Game).GetPetStatsItem(PlayerController(Controller));
		if(StatsItem != None)
		{
			StatsItem.RecallPet(false);
		}
	}
